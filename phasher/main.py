...
menu_font = pygame.font.Font(None, 36)  # Fuente por defecto (36 es el tamaño)
...
# Cargar imágenes de animación de la planta
plant_frames = []
for i in range(1, 14):  # Desde frame1.png hasta frame13.png
    frame = pygame.image.load(f"assets/frames/frame{i}.png")
    frame = pygame.transform.scale(frame, (70, 100))  # Escalar todas las imágenes
    plant_frames.append(frame)


# Posición de la planta (ajustada para estar al nivel del suelo)
plant_x = WIDTH - 100
plant_y = HEIGHT - 90  # Ajustar sumando unos píxeles adicionales para acercarla más al suelo



# Inicialización del jugador y la bala
player = pygame.Rect(50, HEIGHT - 70, 50, 70)  # Jugador
bala = pygame.Rect(plant_x + 20, plant_y + 40, 30, 30)  # Bala alineada con la boca de la planta
...
def reiniciar_modelos():
    ...
    arbol_model = DecisionTreeClassifier()  # Reiniciar el modelo de árbol
    nn_model = None  # Reiniciar la red neuronal (sin inicializar)
...
def visualizar_arbol(modelo, feature_names):
    ...
    graph.view()  # Abre automáticamente la visualización del árbol
